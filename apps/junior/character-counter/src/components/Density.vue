  ...
  <section class="mb-5 flex flex-col gap-3 px-4 md:px-8" aria-labelledby="density-heading">
    ...
    <ul v-else class="flex flex-col gap-3" aria-live="polite">
      <li 
        ...
        :key="item.letter" 
        class="flex items-center gap-3.5 text-preset-4 text-neutral-900 dark:text-neutral-200"
      >
        ...
        <span class="w-24 text-right">
          {{ item.count }}
          <span class="text-neutral-600 dark:text-neutral-400 font-light"> ({{ item.percentage }}%)</span>
        </span>
      </li>
    </ul>
