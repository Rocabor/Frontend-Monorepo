...
<template>
  <button
    ...
    :class="{ 'w-full sm:w-70': centered, 'is-visible': revealed }"
    @click="emit('open', project)"
  >
    <div class="relative h-50 overflow-hidden bg-surface-lowest">
      ...
      <span v-if="rank >= 0" class="absolute bottom-2.5 left-2.5 z-10 font-mono text-[0.55rem] font-bold tracking-wider uppercase px-2.5 py-1 rounded-full bg-[rgba(251,191,36,0.12)] text-inter border border-[rgba(251,191,36,0.3)]">{{ rankLabel }}</span>
      ...
      >
        <svg viewBox="0 0 24 24" width="14" height="14" fill="#e1002d" stroke="#e1002d" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.6 1-1a5.5 5.5 0 0 0 0-7.8z" /></svg>
        <span v-if="likesLoading" class="skeleton-block w-4 h-3 rounded" />
        <span v-else class="text-white">{{ likeCount(project.href) }}</span>
      </button>

      <div class="absolute left-0 right-0 bottom-0 z-2 p-4 flex flex-col gap-3 bg-linear-to-t from-[rgba(10,14,18,0.92)] via-[rgba(10,14,18,0.55)] to-transparent opacity-0 translate-y-3 transition group-hover:opacity-100 group-hover:translate-y-0 max-sm:opacity-100 max-sm:translate-y-0" aria-hidden="true">
        <div class="flex flex-wrap gap-1.5">
          <span
            v-for="tag in project.tags"
            :key="tag"
            class="font-mono text-[0.55rem] uppercase tracking-wider px-2 py-0.5 rounded bg-white/10 text-bright border border-white/10 transition group-hover:-translate-y-0.5 group-hover:bg-newbie group-hover:text-[#04141b] group-hover:border-transparent group-hover:shadow-[0_6px_16px_rgba(6,182,212,0.35)]"
          >{{ tag }}</span>
        </div>
        <div class="flex items-center justify-between">
          <span class="inline-flex items-center gap-1.5 font-mono text-[0.6rem] font-bold tracking-wider text-primary">
            <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor" aria-hidden="true"><path d="M12 2l2.4 7.4H22l-6 4.6 2.3 7.4L12 17l-6.3 4.4L8 14 2 9.4h7.6z" /></svg>
            EXPLORE DEMO
          </span>
          <span class="inline-flex items-center gap-1 font-mono text-[0.6rem] font-bold tracking-wider text-dim">
            <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7z" /><circle cx="12" cy="12" r="3" /></svg>
            <span v-if="viewsLoading" class="skeleton-block w-5 h-3 rounded" />
            <span v-else>{{ viewCount(project.href) }}</span>
          </span>
          <div class="flex gap-2.5 text-dim" aria-hidden="true">
    ...
    <div class="p-4 flex-1 flex items-center justify-center">
      <h3 class="text-center text-[0.95rem] font-bold text-bright m-0 uppercase transition-colors group-hover:text-primary">
        {{ project.title }}
      </h3>
    </div>
  </button>
